...
struct OpenAIProvider: AIProvider {
    ...
    func generatePlan(goal: String, minutes: Int, model: String) async throws -> AIPlanResult {
        ...
        return try await requestPlan(goal: goal, minutes: minutes, model: model, maxTokens: maxOutputTokens, didRetry: false)
    }
    ...
    private func requestPlan(goal: String, minutes: Int, model: String, maxTokens: Int, didRetry: Bool) async throws -> AIPlanResult {
        ...
        do {
            ...
            if normalizedText.isEmpty {
                let statusReason = extractStatusReason(data)
                AILogger.log(type, "status=\(statusReason.status ?? "nil") reason=\(statusReason.reason ?? "nil")")
                if statusReason.status == "incomplete",
                   statusReason.reason == "max_output_tokens",
                   !didRetry {
                    let nextTokens = min(maxTokens * 2, 1200)
                    AILogger.log(type, "retrying with max_output_tokens=\(nextTokens)")
                    return try await requestPlan(goal: goal, minutes: minutes, model: model, maxTokens: nextTokens, didRetry: true)
                }
